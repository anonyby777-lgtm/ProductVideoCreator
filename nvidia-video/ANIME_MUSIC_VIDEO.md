...
1. Put the licensed audio at `public/anime-clip/audio/song.mp3`, then set `HAS_MUSIC_ASSET = true` in `src/anime-clip/AnimeMusicVideo.tsx`.
...
4. Replace every entry in `LYRICS` with the authorized original lyric, Portuguese translation, and precise timestamps. Adjust each scene's `duration`; update `VIDEO_DURATION_SECONDS` and the config duration together if the song is not 180 seconds.
